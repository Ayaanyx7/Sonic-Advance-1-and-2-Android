...
ifeq ($(GAME_NAME), sa1)
	...
	ifeq ($(PLATFORM),sdl)
		...
		CPPFLAGS += -D TITLE_BAR=$(BUILD_NAME).$(PLATFORM) -D PLATFORM_GBA=0 -D PLATFORM_SDL=1 -D PLATFORM_WIN32=0 -Iandroid-project/app/jni/SDL/include -Iandroid-project/app/jni/SDL_image/include
	else ifeq ($(PLATFORM),sdl_psp)
		CC1FLAGS += -G0
...
ifeq ($(PLATFORM),gba)
  ...
  ifeq ($(PLATFORM), sdl)
    ...
    CPP := armv7a-linux-androideabi$(ANDROID_API)-clang -E
